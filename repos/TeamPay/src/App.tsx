...
const App = () => {
  ...
  // --- STATE FOR COMPTEUR D'HEURES ---
  const [timerState, setTimerState] = useState<'inactive' | 'active' | 'paused'>('inactive');
  const [timerStartTime, setTimerStartTime] = useState<number | null>(null);
  const [timerPauseStartTime, setTimerPauseStartTime] = useState<number | null>(null);
  const [timerAccumulatedPause, setTimerAccumulatedPause] = useState<number>(0); // in ms
  const [timerElapsedSeconds, setTimerElapsedSeconds] = useState<number>(0);
  const [timerHistory, setTimerHistory] = useState<any[]>([
    { id: 'h-1', debut: '08:00', pause: '01:00', fin: '17:00', total: '08:00', city: 'Montréal, QC' },
    { id: 'h-2', debut: '09:15', pause: '00:30', fin: '16:45', total: '07:00', city: 'Laval, QC' }
  ]);

  // --- HYBRID TEAM & MAGIC LINK STATES ---
  const [employeeMagicSession, setEmployeeMagicSession] = useState<string | null>(null);
  const [magicLinkModal, setMagicLinkModal] = useState<{ id: string; name: string; url: string } | null>(null);
  const [gpsLogs, setGpsLogs] = useState<any[]>([
    { id: 'gps-1', employeeName: 'Marc Tremblay', action: 'Début de journée (IN)', time: '2026-05-20 08:02:15', city: 'Montréal, QC', coords: '45.5017° N, 73.5673° W' },
    { id: 'gps-2', employeeName: 'Marc Tremblay', action: 'Pause', time: '2026-05-20 12:05:40', city: 'Montréal, QC', coords: '45.5017° N, 73.5673° W' },
    { id: 'gps-3', employeeName: 'Richard Duchesne', action: 'Début de journée (IN)', time: '2026-05-20 07:15:33', city: 'Laval, QC', coords: '45.5699° N, 73.6920° W' },
  ]);
  ...
  useEffect(() => {
    ...
    interval = setInterval(() => {
      const now = Date.now();
      // Update primary admin personal timer if active
      if (timerState === 'active' && timerStartTime !== null) {
        const totalMs = now - timerStartTime - timerAccumulatedPause;
        setTimerElapsedSeconds(Math.max(0, Math.floor(totalMs / 1000)));
      }

      // Live-increment seconds elapsed for ANY working team member (rich simulation)
      setCollaborateurs(prev => prev.map(c => {
        if (c.status === 'working' && c.timerStartTime) {
          const totalMs = now - c.timerStartTime - (c.timerAccumulatedPause || 0);
          return {
            ...c,
            timerElapsedSeconds: Math.max(0, Math.floor(totalMs / 1000))
          };
        }
        return c;
      }));
  ...
  }, [timerState, timerStartTime, timerAccumulatedPause]);
  ...
  const [comptableEmail, setComptableEmail] = useState('natalia.caisse@solutionsgpa.ca');
  ...
  const WorkspaceSidebar = () => {
    ...
    const navItems = [
      ...
      { id: 'compteur-heures', label: "Compteur d'Heures", icon: <Timer size={18} /> },
  ...
  const [collaborateurs, setCollaborateurs] = useState([
    { id: 'COLL-001', name: 'Marc Tremblay', rate: 30, job: 'Peintre sous-traitant', status: 'off', timerStartTime: null, timerPauseStartTime: null, timerAccumulatedPause: 0, timerElapsedSeconds: 0, history: [] },
    { id: 'COLL-002', name: 'Chantal Roy', rate: 22, job: 'Adjointe', status: 'paused', timerStartTime: Date.now() - 18000000, timerPauseStartTime: Date.now() - 1200000, timerAccumulatedPause: 0, timerElapsedSeconds: 16800, history: [] },
    { id: 'COLL-003', name: 'Richard Duchesne', rate: 35, job: 'Menuisier', status: 'working', timerStartTime: Date.now() - 14400000, timerPauseStartTime: null, timerAccumulatedPause: 1800000, timerElapsedSeconds: 12600, history: [] },
  ]);
  const [timeCards, setTimeCards] = useState([
    { id: 'TC-001', companyId: '1', contractorName: 'Marc Tremblay', task: 'Triplex Laval - Peinture', hours: 15, rate: 30, date: '2026-05-18', status: 'En attente' },
    { id: 'TC-002', companyId: '1', contractorName: 'Chantal Roy', task: 'Solutions GPA - Admin', hours: 10, rate: 22, date: '2026-05-15', status: 'Approuvé' },
  ]);
  const [newCollName, setNewCollName] = useState('');
  const [newCollRate, setNewCollRate] = useState('25');
  const [newCollJob, setNewCollJob] = useState('Peintre sous-traitant');
  const [selectedCollId, setSelectedCollId] = useState('');
  const [newHours, setNewHours] = useState('8');
  const [newTask, setNewTask] = useState('Triplex Laval - Peinture');
...
Solutions GPA Inc.`;
  ...
  const handleAddCollaborateur = () => {
    if (!newCollName.trim()) {
      alert("S'il vous plaît, entrez le nom complet de l'employé ou sous-traitant.");
      return;
    }
    const parsedRate = parseFloat(newCollRate);
    if (isNaN(parsedRate) || parsedRate <= 0) {
      alert("Veuillez saisir un taux horaire valide supérieur à 0.");
      return;
    }
    const newId = `COLL-${Date.now()}`;
    setCollaborateurs(prev => [
      ...prev,
      { id: newId, name: newCollName.trim(), rate: parsedRate, job: newCollJob }
    ]);
    setNewCollName('');
    setNewCollRate('25');
    playNotificationSound();
    alert(`🎉 ${newCollName.trim()} enregistré comme ${newCollJob} avec un taux de ${parsedRate}$/h.`);
  };

  const handleAddSimulatedTime = () => {
    const selectedColl = collaborateurs.find(c => c.id === selectedCollId);
    if (!selectedColl) {
      alert("Veuillez sélectionner un employé ou sous-traitant actif.");
      return;
    }
    const hrs = parseFloat(newHours);
    if (isNaN(hrs) || hrs <= 0) {
      alert("Veuillez entrer une quantité d'heures valide.");
      return;
    }
    const newTC = {
      id: `TC-${Date.now()}`,
      companyId: activeCompanyId,
      contractorName: selectedColl.name,
      task: newTask.trim() || 'Entretien général',
      hours: hrs,
      rate: selectedColl.rate,
      date: new Date().toISOString().split('T')[0],
      status: 'En attente'
    };
    setTimeCards(prev => [newTC, ...prev]);
    playNotificationSound();
    alert(`Feuille de temps de ${hrs} heures à ${selectedColl.rate}$/h soumise.`);
  };

  const handleApproveTimeCard = (tcId: string) => {
    const tc = timeCards.find(t => t.id === tcId);
    if (!tc) return;

    // Calculate line total dynamically
    const lineTotal = tc.hours * (tc.rate || 0);

    // Update log status to Approved
    setTimeCards(timeCards.map(t => t.id === tcId ? { ...t, status: 'Approuvé' } : t));

    // CRITICAL: Push directly into global bookkeeping expenses array matching Fabiola's standard
    const automatedExpense = {
      id: Date.now(),
      companyId: activeCompanyId,
      fecha: tc.date,
      fournisseur: tc.contractorName || 'Artisan Sous-traitant',
      cat: "Main d'œuvre / Salaire",
      subtotal: lineTotal,
      tps: 0,
      tvq: 0,
      total: lineTotal,
      lien: null, // Leaves it open for bank statement verification matching
      partnerTag: activeUser
    };

    setDepenses(prev => [automatedExpense, ...prev]);
    playNotificationSound();
    alert("✅ Heures approuvées ! Le salaire a été injecté automatiquement dans votre Tenue de Livres comme dépense de Main-d'œuvre.");
  };
  ...
  // --- INTERCEPTION DE SESSION LIEN MAGIQUE EMPLOYÉ ---
  if (employeeMagicSession) {
    const activeWorker = collaborateurs.find(c => c.id === employeeMagicSession);
    if (!activeWorker) {
      setEmployeeMagicSession(null);
    } else {
      ...
      const GPS_STATIONS = [
        { city: 'Laval, QC', coords: '45.5699° N, 73.6920° W' },
        { city: 'Montréal, QC', coords: '45.5017° N, 73.5673° W' },
        { city: 'Terrebonne, QC', coords: '45.6924° N, 73.6331° W' },
        { city: 'Longueuil, QC', coords: '45.5312° N, 73.5181° W' },
        { city: 'Blainville, QC', coords: '45.6729° N, 73.8821° W' }
      ];

      const handleStartDay = () => {
        const station = GPS_STATIONS[Math.floor(Math.random() * GPS_STATIONS.length)];
        const now = Date.now();
        const dateStr = new Date(now).toISOString().slice(0, 19).replace('T', ' ');
        
        // Save GPS capture log
        const logId = `gps-${Date.now()}`;
        const newLog = {
          id: logId,
          employeeName: activeWorker.name,
          action: 'Début de journée (IN)',
          time: dateStr,
          city: station.city,
      ...
      const handlePauseDay = () => {
        ...
        // Update collaborator stage
        setCollaborateurs(prev => prev.map(c => {
          if (c.id === activeWorker.id) {
            return {
              ...c,
              status: 'paused',
              timerPauseStartTime: now,
            };
          }
          return c;
        }));
      ...
      const handleResumeDay = () => {
        ...
        const now = Date.now();
        const dateStr = new Date(now).toISOString().slice(0, 19).replace('T', ' ');
        const pausedMs = activeWorker.timerPauseStartTime ? (now - activeWorker.timerPauseStartTime) : 0;

        const logId = `gps-${Date.now()}`;
        const newLog = {
          id: logId,
          employeeName: activeWorker.name,
          action: 'Fin de pause (Reprise)',
          time: dateStr,
          city: station.city,
          coords: station.coords,
        };
        ...
        setCollaborateurs(prev => prev.map(c => {
          if (c.id === activeWorker.id) {
            return {
              ...
              status: 'working',
              timerPauseStartTime: null,
              timerAccumulatedPause: (c.timerAccumulatedPause || 0) + pausedMs,
            };
          }
          return c;
        }));
      ...
      const handleEndDay = () => {
        ...
        let finalPausedMs = activeWorker.timerAccumulatedPause || 0;
        if (activeWorker.status === 'paused' && activeWorker.timerPauseStartTime) {
          finalPausedMs += (now - activeWorker.timerPauseStartTime);
        }

        const totalElapsedMs = activeWorker.timerStartTime ? (now - activeWorker.timerStartTime - finalPausedMs) : 0;
        const totalSecs = Math.max(0, Math.floor(totalElapsedMs / 1000));
        
        const h = Math.floor(totalSecs / 3600);
        const m = Math.floor((totalSecs % 3600) / 60);
        const totalStr = `${h.toString().padStart(2, '0')}h ${m.toString().padStart(2, '0')}m`;

        const logId = `gps-${Date.now()}`;
        const newLog = {
          id: logId,
          employeeName: activeWorker.name,
          action: 'Fin de journée (OUT)',
          time: dateStr,
          city: station.city,
          coords: station.coords,
        };
        ...
        // Format dates
        const debutHourStr = activeWorker.timerStartTime ? new Date(activeWorker.timerStartTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '08:00';
        const finHourStr = new Date(now).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

        const formattedDate = new Date(now).toISOString().split('T')[0];

        // Create historic item for worker
        const newHistoryItem = {
          id: `h-${Date.now()}`,
          debut: debutHourStr,
          pause: formatMsToTimeString(finalPausedMs),
          fin: finHourStr,
          total: totalStr,
          city: station.city
        };

        // Inject new global time card for billing
        const newGlobalTimeCard = {
          id: `TC-${Date.now()}`,
          companyId: activeCompanyId,
          contractorName: activeWorker.name,
          task: `Chantier ${station.city} - Travaux Techniques`,
          hours: Number(Math.max(0.1, totalSecs / 3600).toFixed(2)),
          rate: activeWorker.rate,
          date: formattedDate,
          status: 'En attente'
        };

        setTimeCards(prev => [newGlobalTimeCard, ...prev]);
        ...
        playNotificationSound();
        alert(`🏁 Félicitations ! Votre journée de travail de ${totalStr} a été enregistrée à ${station.city} et envoyée pour révision aux administrateurs.`);
      };

      return (
        <div className={`min-h-screen ${darkMode ? 'bg-black text-zinc-100' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans transition-all duration-300 relative max-w-full overflow-x-hidden`}>
          <header className={`px-6 py-5 border-b ${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-200'} flex items-center justify-between sticky top-0 z-50 shadow-sm`} style={{ borderTop: `4px solid #10b981` }}>
            <div className="flex items-center space-x-3">
              <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-xl">
                <ShieldCheck size={20} />
              </div>
              <div className="text-left">
                <h1 className="font-black italic uppercase tracking-tighter text-sm leading-none">AutoCompt Portal</h1>
                <span className="text-[7.5px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest block mt-0.5">Espace Salarié Sécurisé</span>
              </div>
            </div>

            <div className="flex items-center space-x-3">
              <span className="text-[8px] font-black uppercase tracking-wider text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-full italic animate-pulse">
                Lien Magique Actif
              </span>
              <button 
                onClick={() => { setEmployeeMagicSession(null); playNotificationSound(); }}
                className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-black text-[8px] uppercase tracking-wider italic rounded-full border-none transition-all cursor-pointer flex items-center space-x-1"
              >
                <LogOut size={10} />
          ...
          <main className="p-6 flex-1 flex flex-col max-w-md mx-auto w-full space-y-6">
            <div className={`${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} p-8 rounded-[36px] border shadow-xl flex flex-col items-center space-y-4 text-center`}>
              <div className="flex flex-col items-center space-y-1">
                <span className="text-[8.5px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest leading-none">Identité vérifiée</span>
                <h2 className="text-xl font-black uppercase italic tracking-tighter text-[#1F2937] dark:text-white leading-none">{activeWorker.name}</h2>
                <span className="text-[8px] font-black uppercase tracking-wider bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400 px-2.5 py-1 rounded-md mt-1 italic">{activeWorker.job} • {activeWorker.rate} $/h</span>
              </div>

              {/* RESTRICTION WARNING FOR LEGAL AUDIT */}
              <div className="w-full bg-amber-500/5 border border-amber-500/10 p-3 rounded-2xl flex items-start space-x-2 text-left">
                <Shield size={14} className="text-amber-500 shrink-0 mt-0.5" />
                <p className="text-[8px] font-bold text-amber-600/90 leading-tight">
                  MODE RESTREINT : Cette session n'affiche aucun renseignement financier, bancaire, fiscal ou corporatif conformément aux normes d'audit BYOS.
                </p>
              </div>

              {/* PULSING TIMER CARD */}
              <div className="w-full py-4 space-y-1">
                <span className="text-[8px] font-black uppercase tracking-widest text-[#059669] dark:text-emerald-400 animate-pulse block">SESSION EN COURS</span>
                <h1 className="text-4xl font-black italic tracking-tighter font-mono tabular-nums text-slate-800 dark:text-zinc-100">
                  {formatTime(activeWorker.timerElapsedSeconds || 0)}
                </h1>
              </div>

              {/* CORE BREATHING PULSING HERO TRACKING BUTTON */}
              <div className="relative flex items-center justify-center w-48 h-48 my-2">
                {activeWorker.status === 'working' && (
                  <>
                    <div className="absolute inset-0 rounded-full border border-emerald-500/5 animate-ping scale-125 duration-1000" />
                    <div className="absolute inset-3 rounded-full border border-emerald-500/10 animate-pulse scale-110 duration-1000" />
                    <div className="absolute inset-6 rounded-full border border-emerald-500/20 animate-pulse duration-1000" />
                    <div className="absolute inset-9 rounded-full bg-emerald-500/10 blur-[20px] dark:bg-emerald-500/20 shadow-[0_0_40px_rgba(16,185,129,0.25)]" />
                  </>
                )}
                {activeWorker.status === 'paused' && (
                  <>
                    <div className="absolute inset-6 rounded-full border border-amber-500/20 animate-pulse duration-1000" />
                    <div className="absolute inset-9 rounded-full bg-amber-500/10 blur-[20px]" />
                  </>
                )}

                {activeWorker.status === 'off' ? (
                  <button
                    onClick={handleStartDay}
                    className="relative z-10 w-36 h-36 rounded-full bg-[#059669] text-white font-black uppercase tracking-wider italic hover:bg-emerald-600 transition-all active:scale-95 shadow-xl shadow-emerald-500/20 hover:shadow-2xl flex flex-col items-center justify-center space-y-1.5 border-none cursor-pointer"
                  >
                    <Timer size={32} className="animate-spin" style={{ animationDuration: '6s' }} />
                    <span className="text-[9px] leading-tight font-black uppercase">DÉBUTER LA<br />JOURNÉE</span>
                  </button>
                ) : (
                  <button
                    onClick={handleEndDay}
                    className="relative z-10 w-36 h-36 rounded-full bg-rose-600 text-white font-black uppercase tracking-wider italic hover:bg-rose-700 transition-all active:scale-95 shadow-xl shadow-rose-500/25 hover:shadow-2xl flex flex-col items-center justify-center space-y-1.5 border-none cursor-pointer"
                  >
                    <Lock size={32} />
                    <span className="text-[9px] leading-tight font-black uppercase">FINIR LA<br />JOURNÉE</span>
                  </button>
                )}
              </div>
                ...
                <div className="flex w-full space-x-4 mt-2">
                  {activeWorker.status === 'working' ? (
                    <button
                      ...
                      className="flex-1 py-4 bg-amber-500 hover:bg-amber-600 text-white font-black uppercase italic text-[10px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-amber-500/15 border-none cursor-pointer"
                    >
                      Mettre en Pause
                    </button>
                  ) : (
                    <button
                      ...
                      className="flex-1 py-4 bg-[#059669] hover:bg-emerald-600 text-white font-black uppercase italic text-[10px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-emerald-500/15 border-none cursor-pointer"
                    >
                      Reprendre
                    </button>
                  )}
            ...
            {/* DAILY SUMMARY CARD FOR WORKER */}
            <div className={`${darkMode ? 'bg-zinc-900/30' : 'bg-white'} p-6 rounded-[36px] shadow-sm border ${darkMode ? 'border-zinc-900' : 'border-slate-100'} space-y-4`}>
              <div className="flex items-center space-x-2.5">
                <div className="p-2 bg-indigo-500/10 text-indigo-500 rounded-xl">
                  <MapPin size={18} />
                </div>
                <div className="text-left">
                  <h3 className="text-[10px] font-black uppercase italic tracking-wider leading-none">Chantier Actif</h3>
                  <p className="text-[7px] text-slate-400 uppercase font-bold mt-0.5">Données géolocalisées en temps réel</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-left">
                <div className={`p-4 rounded-3xl ${darkMode ? 'bg-zinc-950' : 'bg-slate-50'}`}>
                  <span className="text-[7.5px] font-black text-slate-400 uppercase tracking-widest block leading-none">Heures du jour</span>
                  <span className="text-xs font-black italic text-emerald-500 mt-1 block">
                    {Math.max(0, Number(((activeWorker.timerElapsedSeconds || 0) / 3600).toFixed(3)))} hrs
                  </span>
                </div>
                <div className={`p-4 rounded-3xl ${darkMode ? 'bg-zinc-950' : 'bg-slate-50'}`}>
                  <span className="text-[7.5px] font-black text-slate-400 uppercase tracking-widest block leading-none">Lieu de travail</span>
                  <div className="flex items-center space-x-1 mt-1 text-slate-600 dark:text-zinc-300">
                    <MapPin size={12} className="text-rose-500" />
                    <span className="text-xs font-black truncate">Laval, QC</span>
                  </div>
                </div>
              </div>
            </div>

            {/* LIVE GPS CAPTURE LOGS (TIMELINE) */}
            <div className={`${darkMode ? 'bg-zinc-900/30' : 'bg-white'} p-6 rounded-[36px] shadow-sm border ${darkMode ? 'border-zinc-900' : 'border-slate-100'} space-y-4`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2.5">
                  ...
                  <h3 className="text-[10px] font-black uppercase italic tracking-wider leading-none">Journal GPS (Audit)</h3>
                </div>
                <span className="text-[6.5px] font-black uppercase bg-slate-100 dark:bg-zinc-800 text-slate-500 px-2.5 py-1 rounded">BYOS LEGAL CAPTURE</span>
              </div>

              <div className="space-y-3 max-h-[160px] overflow-y-auto">
                {gpsLogs.filter(log => log.employeeName === activeWorker.name).map((log) => (
                  <div key={log.id} className="p-3 rounded-2xl border border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/40 text-left flex justify-between items-start text-[8px]">
                    <div>
                      <p className="font-black uppercase tracking-tight text-slate-700 dark:text-zinc-200">{log.action}</p>
                      <p className="text-slate-400 dark:text-zinc-500 mt-0.5">{log.time} • <span className="font-mono">{log.coords}</span></p>
                    </div>
                    <span className="bg-emerald-500/10 text-[#059669] px-2 py-0.5 rounded-full font-black uppercase">{log.city}</span>
                  </div>
                ))}
                {gpsLogs.filter(log => log.employeeName === activeWorker.name).length === 0 && (
                  <p className="text-center text-[9px] italic text-slate-400 py-4">Aucune trace de géolocalisation capturée aujourd'hui.</p>
                )}
              </div>
            </div>
          </main>
             ...
             <div className={`p-4 rounded-2xl flex justify-between items-center ${darkMode ? 'bg-zinc-900/40' : 'bg-slate-50/60'} border ${darkMode ? 'border-zinc-800' : 'border-slate-100/80'} text-[10px]`}>
                <div>
                   ...
                   <p className={`font-black italic uppercase transition-colors ${getEffectiveTier() === 'superadmin' ? 'text-amber-500' : 'text-slate-800 dark:text-zinc-200'}`}>
                      {getEffectiveTier() === 'superadmin' ? 'Super Admin' : selectedTier}
                   </p>
                </div>
  ...
  if (vista === 'dashboard') return (
    <div className={`min-h-screen ${darkMode ? 'bg-black text-white' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans text-left leading-relaxed max-w-full overflow-x-hidden md:pl-72 relative transition-all duration-300`}>
      ...
      <main className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => setVista('kilometraje')} className="w-full bg-gradient-to-tr from-emerald-600 to-teal-500 text-white p-5 rounded-[28px] shadow-md flex flex-col justify-between items-start text-left active:scale-95 transition-all border-none cursor-pointer">
            <Car size={24} className="mb-4" />
            <div>
              <p className="text-[11px] font-black uppercase italic tracking-tight">Kilométrage GPS</p>
              <p className="text-[7.5px] opacity-80 font-bold uppercase tracking-tight mt-0.5">Nouveau trajet</p>
            </div>
          </button>

          <button onClick={() => setVista('equipe')} className="w-full bg-slate-900 dark:bg-zinc-900 text-white p-5 rounded-[28px] shadow-md flex flex-col justify-between items-start text-left active:scale-95 transition-all border-none cursor-pointer">
            <Users size={24} className="mb-4 text-emerald-400" />
            <div>
              <p className="text-[11px] font-black uppercase italic tracking-tight">Temps & Équipe</p>
              <p className="text-[7.5px] opacity-80 font-bold uppercase tracking-tight mt-0.5">Suivi Georapport</p>
            </div>
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            {selectedTier === 'gratuit' && (
              <div className="absolute top-4 right-4 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-black tracking-widest uppercase italic text-[7px] px-3 py-1.5 rounded-full shadow-lg border border-amber-300/40 animate-pulse flex items-center space-x-1 z-30">
                <span>👑 LIVE IA CORE - PLAN PREMIUM</span>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-3 text-left">
          ...
          <button onClick={() => setVista('doculegal')} className={`${darkMode ? 'bg-zinc-950 border-zinc-900 text-white' : 'bg-white border-slate-200 text-slate-900'} p-5 rounded-[32px] border shadow-sm flex flex-col items-start space-y-2 text-left hover:border-[#059669] hover:shadow-xl transition-all active:scale-95`}><div className={`${darkMode ? 'bg-indigo-900/20 text-indigo-400' : 'bg-indigo-100 text-indigo-600'} p-3 rounded-2xl`}><FileSignature size={22} /></div><span className="text-[10px] font-black uppercase italic tracking-tighter">DocuLegal</span></button>
          ...
          <button 
            onClick={() => { setVista('heures-paie'); playNotificationSound(); }} 
          ...
          >
            <div className={`${darkMode ? 'bg-blue-900/20 text-blue-400' : 'bg-blue-100 text-blue-600'} p-3 rounded-2xl`}>
              <Timer size={22} />
            </div>
            <span className="text-[10px] font-black uppercase italic tracking-tighter leading-none">Heures<br/>& Paie</span>
            <p className="text-[7px] font-bold text-slate-400 uppercase tracking-tight">Gestion des employés & sous-traitants</p>
          </button>

          <button 
            onClick={() => { setVista('compteur-heures'); playNotificationSound(); }} 
            className={`${darkMode ? 'bg-zinc-950 border-zinc-900 text-white' : 'bg-white border-slate-200 text-slate-900'} p-5 rounded-[32px] border shadow-sm flex flex-col items-start space-y-2 text-left hover:border-[#059669] hover:shadow-xl transition-all active:scale-95`}
          >
            <div className={`${darkMode ? 'bg-emerald-950/20 text-emerald-455' : 'bg-emerald-100 text-[#059669]'} p-3 rounded-2xl`}>
              <Timer size={22} />
            </div>
            <span className="text-[10px] font-black uppercase italic tracking-tighter leading-none">Compteur<br/>d'Heures</span>
            <p className="text-[7px] font-bold text-slate-400 uppercase tracking-tight">Pointeuse de journée en temps réel</p>
          </button>
    ...
    const getFolderColorMap = (name: string) => {
      ...
      if (n.includes('bail') || n.includes('baux')) {
        return { 
          ...
          bgColor: darkMode ? 'bg-emerald-950/20' : 'bg-emerald-50/70', 
    ...
    return (
      <div className={`min-h-screen ${darkMode ? 'bg-black text-white' : 'bg-slate-50 text-slate-900'} flex flex-col animate-in slide-in-from-right text-left font-sans max-w-full overflow-x-hidden pb-12 md:pl-72 relative transition-all duration-300`}>
        ...
        >
          ...
          <span className="text-[7.5px] font-black uppercase text-emerald-600 dark:text-emerald-400 tracking-widest bg-emerald-500/10 px-3 py-1.5 rounded-full italic">Système BYOS Actif 🔑</span>
        </header>
        ...
        <main className={`p-6 flex-1 flex flex-col space-y-6 ${subVistaDocu === 'editor' ? 'max-w-5xl' : 'max-w-lg'} mx-auto w-full transition-all duration-300`}>
          ...
          {subVistaDocu === 'liste' ? (
              ...
              {selectedDocuFolder === null ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className={`p-5 rounded-[28px] border ${darkMode ? 'bg-zinc-900/40 border-zinc-800' : 'bg-white border-slate-200'} shadow-sm flex justify-between items-center`}>
                    <div>
                      <p className="text-[7.5px] font-black uppercase tracking-widest text-[#059669]">Tableau des Signatures</p>
                    ...
                    <div className="flex gap-2">
                       <div className="text-center px-3 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-xl border border-emerald-500/10">
                  ...
                  <div className={`p-5 rounded-[28px] border text-left ${darkMode ? 'bg-zinc-900/50 border-zinc-800' : 'bg-gradient-to-r from-emerald-50/20 to-teal-50/20 border-slate-200'} shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4`}>
                    <div className="space-y-1">
                      <div className="flex items-center space-x-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#059669] animate-pulse" />
                        <span className="text-[7.5px] font-black uppercase tracking-wider text-[#059669]">Votre Profil Électronique</span>
                      </div>
                      <h4 className="text-[11px] font-black italic uppercase leading-tight">
                        {savedSignature || savedInitials ? (
                          <span className="text-emerald-600 dark:text-emerald-400">✓ Signature & Initiales Configures</span>
                        ) : (
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                      ...
                      <button
                        ...
                        className="py-2.5 px-4 bg-[#059669] hover:bg-emerald-600 text-white rounded-xl text-[8.5px] font-black uppercase italic tracking-widest transition-all active:scale-95 shadow-sm flex items-center space-x-1 border-none cursor-pointer"
                      >
                        <PenLine size={13} />
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start">
                             ...
                             <span className={`text-[6px] font-black uppercase px-2 py-0.5 rounded-full ${darkMode ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>Cloud Sync</span>
                          </div>
                          <div>
                  ...
                  <div className="space-y-3 pt-2">
                     <h4 className={`text-[9px] font-black uppercase italic tracking-wider px-1 ${darkMode ? 'text-zinc-500' : 'text-slate-400'}`}>Tous les Documents Récents</h4>
                         ...
                         {companyDocs.map(doc => {
                           return (
                             <div 
                               ...
                               className={`p-4 rounded-[28px] border shadow-sm flex items-center justify-between text-left cursor-pointer transition-all hover:scale-[1.01] ${
                                 darkMode ? 'bg-zinc-950 border-zinc-900 hover:border-zinc-805' : 'bg-white border-slate-150 hover:border-slate-300'
                               }`}
                             >
                               <div className="flex items-center space-x-3 max-w-[70%] text-left">
                                 {/* LUXURY STATUS FORMATTING REVISION #2 */}
                                 {doc.status === 'Signé' ? (
                                 ...
                                 <div className="truncate text-left">
                                   ...
                                   <p className="text-[7px] font-bold text-slate-400 mt-1 uppercase leading-none font-sans">
                                     Partenaire: {doc.recipient} • {doc.date}
                                   </p>
                                 </div>
                               </div>

                               <span className={`px-2 py-0.5 rounded-full text-[6px] font-black uppercase italic ${
                                 doc.status === 'Signé' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'
                               }`}>
                                 {doc.status}
                               </span>
                             </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <button 
                      ...
                      className={`text-[9px] font-black uppercase italic text-[#059669] flex items-center space-x-1.5`}
                    >
                      <ArrowLeft size={12} />
                    ...
                    {docsInFolder.length === 0 ? (
                      <div className={`p-10 text-center border-2 border-dashed rounded-[32px] ${darkMode ? 'border-zinc-900 bg-zinc-950/20 text-zinc-500' : 'border-slate-100 bg-slate-50/50 text-slate-400'}`}>
                        ...
                        <button 
                          ...
                          className="mt-4 px-4 py-3 bg-[#059669] hover:bg-emerald-600 text-white rounded-2xl text-[8px] font-black uppercase italic tracking-widest transition-all shadow-md shadow-emerald-900/10"
                        >
                          + Créer un document
                        </button>
                      </div>
            ...
            <div className="space-y-6 w-full animate-in fade-in duration-300 text-left">
                ...
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className={`p-6 rounded-[32px] border text-left space-y-4 shadow-sm ${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-200'}`}>
                    <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-zinc-900">
                      <p className="text-[9.5px] font-black uppercase text-[#059669] italic">Spécifications de l'Accord</p>
                      {docSimulatedFile && (
                        <span className="text-[7.5px] font-black uppercase bg-teal-500/10 text-teal-605 px-2.5 py-1 rounded-full border border-teal-500/15">
                          📄 {docSimulatedFile}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input 
                          ...
                          className={`w-full p-3.5 rounded-2xl outline-none text-[11px] font-bold transition-all border ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-100 focus:border-[#059669]' : 'bg-slate-50 border-slate-105 focus:border-[#059669]'}`}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-35">
                        <div>
                          ...
                          <select
                            ...
                            className={`w-full p-3.5 rounded-2xl outline-none text-[11px] font-bold transition-all border ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-300 focus:border-[#059669]' : 'bg-slate-50 border-slate-105 focus:border-[#059669]'}`}
                          >
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            value={activeUser}
                            disabled
                            className={`w-full p-3.5 rounded-2xl outline-none text-[11px] font-bold transition-all border opacity-60 ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-slate-50 border-slate-105 text-slate-400'}`}
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea 
                          ...
                          rows={6}
                          className={`w-full p-3.5 rounded-2xl outline-none text-[10.5px] font-mono font-bold transition-all border resize-none leading-relaxed ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-100 focus:border-[#059669]' : 'bg-slate-50 border-slate-105 focus:border-[#059669]'}`}
                  ...
                  <div className={`p-6 rounded-[32px] border text-left space-y-4 shadow-sm ${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-200'}`}>
                    <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-zinc-900">
                      <div>
                        <p className="text-[9.5px] font-black uppercase text-[#059669] italic">Placement Visuel de Validation</p>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-900/30 border border-slate-100 dark:border-zinc-850/40">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          { name: 'Natalia Lopez', colorClass: 'border-emerald-500/35 text-emerald-500 bg-emerald-500/5', dotClass: 'bg-emerald-500' },
                          { name: 'Fabiola Villegas', colorClass: 'border-purple-500/35 text-purple-500 bg-purple-500/5', dotClass: 'bg-purple-500' },
                          { name: docFormRecipient || 'Richard (Destinataire)', colorClass: 'border-amber-500/35 text-amber-600 dark:text-amber-400 bg-amber-500/5', dotClass: 'bg-amber-500' }
                        ].map((sig) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center justify-center space-x-1.5 p-2 rounded-xl text-[8.5px] font-black uppercase italic tracking-tight border transition-all ${
                                isSelected 
                                  ? `${sig.colorClass} border-2 scale-[1.03] ring-1 ring-[#059669]/20` 
                                  : 'bg-transparent border-slate-150 dark:border-zinc-800 text-slate-400 dark:text-zinc-500'
                              }`}
                            >
                              <div className={`w-1.5 h-1.5 rounded-full ${sig.dotClass}`} />
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ...
                        { type: 'Date de signature', label: 'Date', icon: <Calendar size={13} />, textClass: 'text-[#059669]', borderClass: 'border-emerald-500/20 bg-emerald-500/[0.02]', hClass: 'hover:bg-emerald-500/5' }
                      ].map((tool) => (
                        <button
                          key={tool.type}
                          type="button"
                          onClick={() => {
                            ...
                            const roleColor = docSignerActive.includes('Fabiola') ? 'Purple' : docSignerActive.includes('Natalia') ? 'Emerald' : 'Amber';
                            const nextFields = [
                              ...docPlacedFields,
                        ...
                        >
                          <div className={`${tool.textClass} mb-1 animate-pulse`}>{tool.icon}</div>
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className={`p-6 rounded-[32px] border text-left space-y-4 shadow-sm ${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-205'}`}>
                    <div className="flex items-center justify-between border-b pb-3 border-slate-150 dark:border-zinc-900">
                      <div>
                        <p className="text-[9.5px] font-black uppercase text-[#059669] italic font-sans">Signataires & Rôles</p>
                    ...
                    <div className="space-y-2.5">
                      {/* Fabiola */}
                      <div className={`p-3 rounded-2xl border flex items-center justify-between transition-all ${darkMode ? 'bg-zinc-900/40 border-zinc-900/40' : 'bg-slate-50/50 border-slate-100'}`}>
                        <div className="flex items-center space-x-2.5 min-w-0 text-left">
                          <span className="w-2.5 h-2.5 rounded-full bg-purple-500 shadow-md shadow-purple-500/20 shrink-0" />
                          <div className="truncate text-left">
                            <p className="text-[9.5px] font-black leading-none uppercase text-left">Fabiola Villegas</p>
                            <p className="text-[7px] text-slate-400 uppercase font-semibold mt-0.5 truncate text-left">fabiola@propiosolutions.com</p>
                          </div>
                        </div>
                        <span className="text-[6px] font-bold uppercase tracking-widest bg-purple-500/10 text-purple-605 px-2 py-0.5 rounded-full shrink-0">Auteur</span>
                      </div>

                      {/* Natalia */}
                      <div className={`p-3 rounded-2xl border flex items-center justify-between transition-all ${darkMode ? 'bg-zinc-900/40 border-zinc-900/40' : 'bg-slate-50/50 border-slate-100'}`}>
                        <div className="flex items-center space-x-2.5 min-w-0 text-left">
                          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-md shadow-emerald-500/20 shrink-0" />
                          <div className="truncate text-left text-left">
                            <p className="text-[9.5px] font-black leading-none uppercase text-left">Natalia Lopez</p>
                            <p className="text-[7px] text-slate-400 uppercase font-semibold mt-0.5 truncate text-left">natalia@propiosolutions.com</p>
                          </div>
                        </div>
                        <span className="text-[6px] font-bold uppercase tracking-widest bg-emerald-500/10 text-emerald-600 px-2 py-0.5 rounded-full shrink-0">Témoin</span>
                      </div>

                      {/* Recipient / Custom user */}
                      <div className={`p-3 rounded-2xl border-2 border-dashed flex items-center justify-between transition-all ${
                        docFormRecipient 
                          ? (darkMode ? 'bg-zinc-900/60 border-amber-500/20 text-zinc-100' : 'bg-amber-500/[0.02] border-amber-500/20 text-slate-800') 
                          : 'opacity-40 bg-transparent border-slate-200 dark:border-zinc-800'
                      }`}>
                        <div className="flex items-center space-x-2.5 min-w-0 text-left">
                          <span className="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-md shadow-amber-500/20 shrink-0 animation-pulse" />
                          <div className="truncate text-left text-left">
                            <p className="text-[9.5px] font-black leading-none uppercase text-left">{docFormRecipient || "Destinataire principal"}</p>
                            <p className="text-[7px] text-slate-400 uppercase font-semibold mt-0.5 truncate text-left">{docFormEmail || "Courriel à renseigner"}</p>
                          </div>
                        </div>
                        <span className="text-[6px] font-bold uppercase tracking-widest bg-amber-500/10 text-amber-600 px-2 py-0.5 rounded-full shrink-0">Signataire</span>
                      </div>
                    </div>

                    {/* INPUTS FOR SIGNER MANAGEMENT */}
                    <div className="space-y-3 pt-3 border-t border-slate-105 dark:border-zinc-900">
                      <div>
                        <label className="text-[8px] font-black uppercase text-slate-400 italic block mb-1">Nom du destinataire</label>
                        <input 
                          type="text" 
                      ...
                      <div>
                        <label className="text-[8px] font-black uppercase text-slate-400 italic block mb-1">Email destinataire</label>
                        <input 
                          type="email" 
                          value={docFormEmail}
                      ...
                      <div className="grid grid-cols-2 gap-3 items-end">
                        <div>
                          ...
                          <input 
                            ...
                            className={`w-full p-3.5 rounded-2xl outline-none text-[11px] font-mono font-bold transition-all border ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-100 focus:border-[#059669]' : 'bg-slate-50 border-slate-105 focus:border-[#059669]'}`}
                          />
                        </div>
                        <button 
                          type="button"
                          onClick={() => setDocFormSmsVerify(!docFormSmsVerify)}
                          className={`w-full p-3.5 rounded-2xl text-[8px] font-black uppercase italic tracking-tighter transition-all border ${docFormSmsVerify ? (darkMode ? 'bg-emerald-950/30 text-emerald-400 border-emerald-900/40' : 'bg-emerald-55/15 text-emerald-600 border-emerald-100') : (darkMode ? 'bg-zinc-900 text-zinc-600 border-zinc-800' : 'bg-slate-50 text-slate-400 border-slate-100')}`}
                        >
                      ...
                      <div>
                        ...
                        <textarea 
                          ...
                          rows={2}
                          className={`w-full p-3.5 rounded-2xl outline-none text-[10.5px] font-bold transition-all border resize-none ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-100 focus:border-[#059669]' : 'bg-slate-50 border-slate-105 focus:border-[#059669]'}`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3 border-t border-slate-105 dark:border-zinc-900">
                 <button
                   type="button"
                   onClick={() => {
          ...
          <div className="pt-8 mt-auto border-t border-slate-200 dark:border-zinc-900 text-left space-y-1">
             ...
             <ul className="space-y-1 border-l-2 border-emerald-500/30 pl-3">
                ...
                <li className="text-[8px] font-bold text-slate-500 dark:text-zinc-404 uppercase tracking-tight">• Liaison directe entre les baux signés et le Grand Livre fiscal AutoCompt.</li>
                <li className="text-[8px] font-bold text-slate-500 dark:text-zinc-405 uppercase tracking-tight">• Synchronisation multi-période et stockage décentralisé Zero-Knowledge.</li>
             </ul>
          </div>
        </main>
          ...
          {showQrModal && qrModalDoc && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 text-left">
              <motion.div
                ...
                className={`w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden border p-8 flex flex-col items-center text-center space-y-6 animate-in zoom-in-95 duration-300 ${
                  darkMode ? 'bg-zinc-950 border-zinc-900 text-zinc-100' : 'bg-white border-slate-150 text-slate-900'
                }`}
              >
                {/* Header title */}
                <div className="space-y-1">
                   <h3 className="text-sm font-black uppercase italic tracking-tighter text-[#059669]">Signature Mobile d'AutoCompt</h3>
                   <p className="text-[8px] font-black uppercase tracking-wider text-slate-400">Canal crypté de signature cellulaire</p>
                </div>

                {/* DYNAMIC QR CODE CONTAINER WITH THEMED BADGE */}
                ...
                <div className="bg-emerald-500/10 p-4 rounded-2xl border border-emerald-500/15 max-w-xs">
                  <p className="text-[9px] font-black uppercase text-emerald-800 dark:text-emerald-400 leading-normal tracking-wide text-center">
                    Scannez avec votre cellulaire pour signer instantanément avec votre doigt
                  </p>
                </div>

                {/* Recipient verification inputs shown in modal */}
                <div className="w-full space-y-3.5 text-left border-t border-b border-slate-100 dark:border-zinc-900 py-4">
                  <div>
                    <label className="text-[7.5px] font-black uppercase text-slate-400 italic block mb-1">Destinataire SMS portable</label>
                    <input 
                      type="text" 
                      value={qrModalDoc.recipientPhone || '514-555-1234'}
                      disabled
                      className={`w-full p-3.5 rounded-xl text-xs font-mono font-bold border outline-none opacity-80 ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
                    />
                  </div>

                  <div>
                    <label className="text-[7.5px] font-black uppercase text-slate-400 italic block mb-1">Invitation courriel (DocuHub style)</label>
                    <textarea 
                      value={qrModalDoc.emailInvite || `Bonjour, veuillez signer le document ${qrModalDoc.name}.`}
                      disabled
                      rows={2}
                      className={`w-full p-3.5 rounded-xl text-[10px] font-bold border outline-none opacity-80 resize-none ${darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
                    />
                  </div>

                  <div className="flex items-center space-x-2 text-[10px]">
                     <div className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></div>
                     <span className="font-extrabold uppercase italic tracking-widest text-[#059669]">Attente de détection cellulaire</span>
                  </div>
                </div>
                ...
                <div className="w-full space-y-2">
                   <button
                     onClick={() => {
                       ...
                       if (isNew) {
                         setDocuLegalList([signedDoc, ...docuLegalList]);
                       } else {
                       ...
                       setSubVistaDocu('liste');
                       setSelectedDocuFolder(qrModalDoc.cat); // Keep user in that folder
                       playNotificationSound();
                       alert(`🎉 Succès ! Le document "${qrModalDoc.name}" a été officiellement signé sur cellulaire par ${qrModalDoc.recipient} et archivé conformément à la Loi 25.`);
                     }}
                     className="w-full py-4 bg-[#059669] hover:bg-emerald-600 text-white rounded-2xl text-[9.5px] font-black uppercase italic tracking-widest shadow-lg flex items-center justify-center space-x-2 border-none"
                   >
                     <span>Simuler la signature sur cellulaire 📱</span>
                   </button>
                   ...
                   <button
                     ...
                     className={`w-full py-3 text-[9px] font-black uppercase italic rounded-2xl border transition-all ${
                       darkMode 
                         ? 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-zinc-200' 
                         : 'bg-white border-slate-205 text-slate-705 hover:bg-slate-50'
                     }`}
                   >
                     Envoyer l'invitation & Fermer
                   </button>
                </div>
  ...
  if (vista === 'heures-paie') {
    const activeTimeCards = timeCards.filter(tc => tc.companyId === activeCompanyId);
    const totalGrossPayroll = activeTimeCards.reduce((acc, curr) => acc + (curr.hours * curr.rate), 0);

    return (
      <div className={`min-h-screen ${darkMode ? 'bg-black text-zinc-105' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans text-left animate-in slide-in-from-right max-w-full overflow-x-hidden md:pl-72 relative transition-all duration-300`}>
        <WorkspaceSidebar />
        <header 
          className={`${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-200'} px-6 py-5 border-b flex items-center justify-between shadow-sm text-left sticky top-0 z-50`}
          style={{ borderTop: `4px solid ${darkMode ? 'rgba(16, 185, 129, 0.2)' : 'rgba(16, 185, 129, 0.3)'}` }}
        >
          <div className="flex items-center space-x-3">
            ...
            <h2 className="font-black uppercase italic tracking-tighter text-lg leading-none">Heures & Paie</h2>
          </div>
          <span className="text-[8px] font-black uppercase tracking-wider text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-full italic animate-pulse">Mode Premium Actif</span>
        </header>

        <main className="p-6 flex-1 flex flex-col space-y-6 max-w-lg mx-auto w-full">
          {/* STATS BANNER: MASSE SALARIALE */}
          <div className={`p-6 rounded-[36px] ${darkMode ? 'bg-zinc-900/40 border-zinc-900' : 'bg-gradient-to-tr from-[#0F172A] to-slate-800 text-white border-slate-200'} border shadow-xl flex flex-col items-start space-y-1 relative overflow-hidden`}>
            <div className="absolute right-6 top-6 text-emerald-500 opacity-20">
               <Timer size={44} />
            </div>
            <p className={`text-[8px] font-black uppercase italic tracking-widest ${darkMode ? 'text-zinc-400' : 'text-slate-300'}`}>Masse salariale de la semaine</p>
            <p className="text-3xl font-black italic tracking-tighter text-emerald-400">{totalGrossPayroll.toFixed(2)} $</p>
            <div className="flex items-center space-x-2 mt-2">
               <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-ping"></div>
               <span className={`text-[7px] font-bold uppercase tracking-wide ${darkMode ? 'text-zinc-500' : 'text-slate-400'}`}>Mise à jour automatique • {activeTimeCards.length} feuille(s) de temps</span>
            </div>
          </div>

          {/* ADD COLLABORATOR FORM */}
          <div className={`${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} p-6 rounded-[36px] border shadow-sm space-y-4`}>
            <div className="flex items-center space-x-2.5">
              <div className="p-2 bg-blue-500/10 text-blue-500 rounded-xl">
                 <Plus size={18} />
              </div>
              <div>
                 <h3 className="text-[10px] font-black uppercase italic tracking-wider leading-none">Nouveau Collaborateur</h3>
                 <span className="text-[7px] font-bold uppercase text-slate-400">Enregistrer un employé ou sous-traitant</span>
              </div>
            </div>

            <div className="space-y-3">
              <div>
                <label className="text-[7px] font-black uppercase text-slate-400 tracking-wider block mb-1">Nom complet</label>
                <input 
                  type="text" 
                  placeholder="ex: Richard Duchesne"
                  value={newCollName}
                  onChange={(e) => setNewCollName(e.target.value)}
                  className={`w-full p-4 rounded-2xl outline-none text-xs font-bold transition-all border ${darkMode ? 'bg-zinc-950 border-zinc-800 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 focus:border-blue-500'}`}
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[7px] font-black uppercase text-slate-400 tracking-wider block mb-1">Taux horaire ($ / h)</label>
                  <input 
                    type="number" 
                    placeholder="ex: 35"
                    value={newCollRate}
                    onChange={(e) => setNewCollRate(e.target.value)}
                    className={`w-full p-4 rounded-2xl outline-none text-xs font-bold transition-all border ${darkMode ? 'bg-zinc-950 border-zinc-800 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 focus:border-blue-500'}`}
                  />
                </div>
                <div>
                  <label className="text-[7px] font-black uppercase text-slate-400 tracking-wider block mb-1">Métier / Rôle</label>
                  <select
                    value={newCollJob}
                    onChange={(e) => setNewCollJob(e.target.value)}
                    className={`w-full p-4 rounded-2xl outline-none text-xs font-bold transition-all border ${darkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-300 focus:border-blue-500' : 'bg-slate-50 border-slate-200 focus:border-blue-500'}`}
                  >
                    <option value="Peintre sous-traitant">Peintre sous-traitant</option>
              ...
              <button
                ...
                className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase italic text-[9px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-blue-500/10 mt-2"
              >
                Enregistrer d'un clic
              </button>
            </div>
          </div>

          {/* SIMULATE HOURS LOGGER LIST (INTERACTIVE PLAYGROUND FOR NATALIA) */}
          <div className={`${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} p-6 rounded-[36px] border shadow-sm space-y-4`}>
            <div className="flex items-center space-x-2.5">
              <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-xl">
                 <PlusCircle size={18} />
              ...
              <div>
                <h3 className="text-[10px] font-black uppercase italic tracking-wider leading-none">Journal des Heures</h3>
                <span className="text-[7px] font-bold uppercase text-slate-400">Saisir une feuille de temps prestataire</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[7px] font-black uppercase text-slate-400 block mb-1">Bénéficiaire</label>
                <select
                  value={selectedCollId}
                  onChange={(e) => setSelectedCollId(e.target.value)}
                  className={`w-full p-4 rounded-2xl outline-none text-xs font-bold transition-all border ${darkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-300 focus:border-emerald-500' : 'bg-slate-50 border-slate-200 focus:border-emerald-500'}`}
                >
                  <option value="">Sélectionner...</option>
                  {collaborateurs.map(c => (
                    <option key={c.id} value={c.id}>{c.name} ({c.rate}$/h)</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="text-[7px] font-black uppercase text-slate-400 block mb-1">Heures travaillées</label>
                <input 
                  type="number"
                  value={newHours}
                  onChange={(e) => setNewHours(e.target.value)}
                  className={`w-full p-4 rounded-2xl outline-none text-xs font-bold transition-all border ${darkMode ? 'bg-zinc-950 border-zinc-800 text-white focus:border-emerald-500' : 'bg-slate-50 border-slate-200 focus:border-emerald-500'}`}
                />
              </div>
            </div>

            <div>
              <label className="text-[7px] font-black uppercase text-slate-400 block mb-1">Chantier / Tâche liée</label>
              <input 
                type="text"
                placeholder="ex: Triplex Laval - Peinture de l'Unité 3"
                value={newTask}
            ...
            <button
              onClick={handleAddSimulatedTime}
              className="w-full py-4 bg-[#059669] hover:bg-emerald-600 text-white font-black uppercase italic text-[9px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-emerald-500/10"
            >
              Soumettre au Dossier Fiscal
            </button>
          </div>

          {/* VERITCAL TIME CARD TRACKER */}
          <div className="space-y-4">
             <h3 className="text-[10px] font-black uppercase italic text-slate-400 tracking-wider px-1">Feuilles de temps de la semaine</h3>
             
             {activeTimeCards.length === 0 ? (
               <div className="text-center py-10">
                  <p className="text-xs font-bold italic text-slate-400">Aucune feuille d'heures enregistrée pour cette compagnie.</p>
               </div>
             ) : (
               <div className="space-y-3">
                  {activeTimeCards.map(tc => {
                    const totalCost = tc.hours * tc.rate;
                    return (
                      <div 
                        key={tc.id}
                        className={`${darkMode ? 'bg-zinc-900/40 border-zinc-900/80 text-zinc-100' : 'bg-white border-slate-150 text-slate-900'} p-5 rounded-[32px] border shadow-sm flex flex-col space-y-3 transition-all`}
                      >
                        <div className="flex justify-between items-start">
                           <div>
                              <p className="text-[11px] font-black uppercase italic tracking-tighter leading-none">{tc.task}</p>
                              <p className="text-[8px] font-bold text-slate-400 mt-1.5 font-sans">Prestataire : {tc.contractorName}</p>
                           </div>
                           <span className={`px-2.5 py-1 rounded-full text-[6px] font-black uppercase italic leading-none ${
                             tc.status === 'Approuvé' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500 animate-pulse'
                           }`}>
                             {tc.status}
                           </span>
                        </div>

                        <div className={`p-3 rounded-2xl flex justify-between items-center ${darkMode ? 'bg-zinc-950' : 'bg-slate-50'}`}>
                           <p className="text-[8px] font-black text-slate-400 uppercase tracking-wider font-sans">{tc.hours}h × {tc.rate} $ / h</p>
                           <p className="text-xs font-black italic text-emerald-500">{totalCost.toFixed(2)} $</p>
                        </div>

                        {tc.status === 'En attente' && (
                           <button
                             onClick={() => handleApproveTimeCard(tc.id)}
                             className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[8px] uppercase italic tracking-widest rounded-xl transition-all active:scale-95 shadow-md shadow-emerald-500/10 flex items-center justify-center space-x-1.5"
                           >
                             <CheckCircle2 size={12} />
                             <span>Approuver & Concilier</span>
                           </button>
                        )}
                      </div>
                    );
  ...
  // --- MÓDULO COMPTEUR D'HEURES ---
  if (vista === 'compteur-heures') {
    const formatTime = (totalSeconds: number) => {
      const h = Math.floor(totalSeconds / 3600);
      const m = Math.floor((totalSeconds % 3600) / 60);
      const s = totalSeconds % 60;
    ...
    const handleStartDay = () => {
      ...
      setTimerAccumulatedPause(0);
      setTimerElapsedSeconds(0);
      setTimerPauseStartTime(null);
      playNotificationSound();
    };

    const handlePauseDay = () => {
      setTimerState('paused');
      setTimerPauseStartTime(Date.now());
      playNotificationSound();
    };

    const handleResumeDay = () => {
      if (timerPauseStartTime !== null) {
        const pausedMs = Date.now() - timerPauseStartTime;
        setTimerAccumulatedPause(prev => prev + pausedMs);
      }
      setTimerState('active');
      setTimerPauseStartTime(null);
      playNotificationSound();
    ...
    const handleEndDay = () => {
      ...
      let finalPausedMs = timerAccumulatedPause;
      if (timerState === 'paused' && timerPauseStartTime !== null) {
        finalPausedMs += (finMs - timerPauseStartTime);
      }

      const debutStr = new Date(timerStartTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      const finStr = new Date(finMs).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      
      const totalElapsedMs = finMs - timerStartTime - finalPausedMs;
      const totalSecs = Math.max(0, Math.floor(totalElapsedMs / 1000));
      ...
      const pauseStr = formatMsToTimeString(finalPausedMs);
    ...
    return (
      <div className={`min-h-screen ${darkMode ? 'bg-black text-zinc-100' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans text-left animate-in slide-in-from-right max-w-full overflow-x-hidden md:pl-72 relative transition-all duration-300`}>
        <WorkspaceSidebar />
        <header 
          className={`${darkMode ? 'bg-zinc-950 border-zinc-900' : 'bg-white border-slate-200'} px-6 py-5 border-b flex items-center justify-between shadow-sm text-left sticky top-0 z-50`}
          style={{ borderTop: `4px solid ${darkMode ? 'rgba(16, 185, 129, 0.2)' : 'rgba(16, 185, 129, 0.3)'}` }}
        >
          <div className="flex items-center space-x-3">
            ...
            <button onClick={() => setVista('dashboard')} className={`p-2 transition-colors rounded-xl transition-all ${darkMode ? 'text-zinc-500 hover:text-white hover:bg-zinc-900' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50'}`}><ArrowLeft size={24} /></button>
            <h2 className="font-black uppercase italic tracking-tighter text-lg leading-none">Compteur d'Heures</h2>
          </div>
          <span className="text-[8px] font-black uppercase tracking-wider text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-full italic animate-pulse">Pointeuse IA Active</span>
        </header>

        <main className="p-6 flex-1 flex flex-col space-y-6 max-w-md mx-auto w-full">
          {/* THE CHECK-IN / CHECK-OUT CARD */}
          <div className={`${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} p-8 rounded-[36px] border shadow-xl flex flex-col items-center space-y-8 relative overflow-hidden`}>
            
            {/* PASTEL STATUS BUBBLE INDICATOR */}
            <div className="flex justify-center w-full">
              {timerState === 'active' && (
                <div className="bg-emerald-500/10 text-[#059669] px-4 py-2 rounded-full flex items-center space-x-2 border border-emerald-500/20 shadow-sm animate-pulse">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#059669] animate-ping" />
                  <span className="text-[9px] font-black uppercase tracking-widest italic">Session Active</span>
                </div>
              )}
              {timerState === 'paused' && (
                <div className="bg-amber-500/10 text-amber-550 px-4 py-2 rounded-full flex items-center space-x-2 border border-amber-500/20 shadow-sm">
                  <div className="w-2.5 h-2.5 rounded-full bg-amber-500" />
                  <span className="text-[9px] font-black uppercase tracking-widest italic">Session en Pause</span>
                </div>
              )}
              {timerState === 'inactive' && (
                <div className="bg-slate-500/10 text-slate-550 px-4 py-2 rounded-full flex items-center space-x-2 border border-slate-500/20 shadow-sm">
                  <div className="w-2.5 h-2.5 rounded-full bg-slate-400" />
                  <span className="text-[9px] font-black uppercase tracking-widest italic">Session Inactive</span>
                </div>
              )}
            </div>
            ...
            <div className="text-center space-y-1">
              <span className="text-[8px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-550 leading-none">Temps écoulé aujourd'hui</span>
              <h1 className="text-5xl font-black italic tracking-tighter font-mono text-[#059669] dark:text-emerald-450 tabular-nums">
                {formatTime(timerElapsedSeconds)}
              </h1>
            </div>

            {/* LARGE CENTRAL PULSE-EFFECT START BUTTON */}
            <div className="relative flex items-center justify-center w-48 h-48">
              {timerState === 'active' && (
                <>
                  <div className="absolute inset-0 rounded-full border border-emerald-500/5 animate-pulse scale-125" style={{ animationDelay: '400ms', animationDuration: '3s' }} />
                  <div className="absolute inset-3 rounded-full border border-emerald-500/10 animate-pulse scale-110" style={{ animationDelay: '200ms', animationDuration: '2.5s' }} />
                  <div className="absolute inset-6 rounded-full border border-emerald-500/25 animate-pulse" style={{ animationDuration: '2s' }} />
                  <div className="absolute inset-9 rounded-full bg-emerald-500/10 blur-[20px] dark:bg-emerald-500/20 shadow-[0_0_40px_rgba(16,185,129,0.25)]" />
                </>
              ...
              {timerState === 'inactive' ? (
                <button
                  onClick={handleStartDay}
                  className="relative z-10 w-36 h-36 rounded-full bg-[#059669] text-white font-black uppercase tracking-wider italic hover:bg-emerald-600 transition-all active:scale-95 shadow-xl shadow-emerald-500/20 hover:shadow-2xl flex flex-col items-center justify-center space-y-1 border-none cursor-pointer animate-in fade-in zoom-in"
                >
                  <Timer size={32} />
                  <span className="text-[10px] leading-tight mt-1">Débuter la<br/>journée</span>
                </button>
              ) : (
                <button
                  onClick={handleEndDay}
                  className="relative z-10 w-36 h-36 rounded-full bg-rose-600 text-white font-black uppercase tracking-wider italic hover:bg-rose-700 transition-all active:scale-95 shadow-xl shadow-rose-500/25 hover:shadow-2xl flex flex-col items-center justify-center space-y-1 border-none cursor-pointer animate-in fade-in zoom-in"
                >
                  <Lock size={32} />
                  <span className="text-[10px] leading-tight mt-1">Finir la<br/>journée</span>
                </button>
              )}
            </div>

            {/* CONTROLS AREA WITH LARGE GRIP THUMB-FRIENDLY BUTTONS */}
            {timerState !== 'inactive' && (
              <div className="flex w-full space-x-4">
                {timerState === 'active' ? (
                  <button
                    onClick={handlePauseDay}
                    className="flex-1 py-4 bg-amber-550 hover:bg-amber-600 text-white font-black uppercase italic text-[11px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-amber-500/15 border-none cursor-pointer"
                  >
                    Mettre en Pause
                  </button>
                ) : (
                  <button
                    onClick={handleResumeDay}
                    className="flex-1 py-4 bg-[#059669] hover:bg-emerald-600 text-white font-black uppercase italic text-[11px] tracking-widest rounded-2xl transition-all active:scale-95 shadow-lg shadow-emerald-500/15 border-none cursor-pointer"
                  >
                    Reprendre
                  </button>
                )}
          ...
          {/* HISTORIQUE DE LA JOURNÉE TABLE */}
          <div className={`${darkMode ? 'bg-zinc-900/30' : 'bg-white'} p-6 rounded-[36px] shadow-sm border ${darkMode ? 'border-zinc-900' : 'border-slate-100'} space-y-4`}>
            <div className="flex items-center space-x-2.5">
              <div className="p-2 bg-emerald-500/10 text-[#059669] rounded-xl">
                <ClipboardList size={18} />
              </div>
              <h3 className="text-[11px] font-black uppercase italic tracking-wider leading-none">Historique de la journée</h3>
            </div>

            <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-zinc-800">
              <table className="w-full text-left border-collapse text-[10px]">
                <thead>
                  <tr className={`${darkMode ? 'bg-zinc-950 text-zinc-550' : 'bg-slate-50 text-slate-400'} font-black uppercase italic text-[8.5px]`}>
                    <th className="py-3 px-4">Début</th>
                    <th className="py-3 px-4">Pause</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                  {timerHistory.map((item) => (
                    <tr key={item.id} className={`${darkMode ? 'hover:bg-zinc-900/40 text-zinc-300' : 'hover:bg-slate-50/50 text-slate-700'} font-bold`}>
                      <td className="py-3 px-4 font-mono">{item.debut}</td>
                      <td className="py-3 px-4 font-mono text-amber-500">{item.pause}</td>
                      <td className="py-3 px-4 font-mono">{item.fin}</td>
                      <td className="py-3 px-4 font-mono text-[#059669] dark:text-emerald-400 text-right">{item.total}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  // --- MÓDULO ÉQUIPE / TEAM TRACKING & MAGIC LINKS ---
  if (vista === 'equipe') {
    const formatTime = (totalSeconds: number) => {
      const h = Math.floor(totalSeconds / 3600);
      const m = Math.floor((totalSeconds % 3600) / 60);
    ...
    const numWorking = collaborateurs.filter(c => c.status === 'working').length;
    const numPaused = collaborateurs.filter(c => c.status === 'paused').length;
    const numOff = collaborateurs.filter(c => c.status === 'off').length;

    const handleGenerateMagicLink = (emp: any) => {
      const simulatedUrl = `https://autocompt.byos.ai/login/magic?token=byos-secure-${emp.id}-${Date.now().toString().slice(-4)}`;
      setMagicLinkModal({
        id: emp.id,
        name: emp.name,
        url: simulatedUrl
    ...
    return (
      <div className={`min-h-screen ${darkMode ? 'bg-black text-zinc-100' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans text-left animate-in fade-in duration-300 max-w-full overflow-x-hidden md:pl-72 relative transition-all duration-300`}>
        <WorkspaceSidebar />
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <button onClick={() => setVista('dashboard')} className={`p-2 transition-all rounded-xl ${darkMode ? 'text-zinc-500 hover:text-white hover:bg-zinc-900' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50'}`}><ArrowLeft size={24} /></button>
            <h2 className="font-black header-title-custom italic uppercase tracking-tighter text-lg leading-none">Gestion de l'Équipe</h2>
          </div>
          <span className="text-[8px] font-black uppercase tracking-wider text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-full italic animate-pulse">Portail Propriétaire</span>
        </header>

        <main className="p-6 flex-1 flex flex-col space-y-6 max-w-lg mx-auto w-full">
          {/* TEAM METRIC WIDGET BLOCK */}
          <div className="grid grid-cols-3 gap-3">
            <div className={`p-4 rounded-[28px] border relative ${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} flex flex-col items-center justify-center text-center space-y-1`}>
              <div className="flex items-center space-x-1.5">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                <span className="text-sm font-black italic text-emerald-500 leading-none">{numWorking}</span>
              </div>
              <span className="text-[7.5px] font-black uppercase tracking-widest text-slate-405">Actifs</span>
            </div>
            <div className={`p-4 rounded-[28px] border ${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} flex flex-col items-center justify-center text-center space-y-1`}>
              <div className="flex items-center space-x-1.5">
                <div className="w-2 h-2 rounded-full bg-amber-500" />
                <span className="text-sm font-black italic text-amber-500 leading-none">{numPaused}</span>
              </div>
              <span className="text-[7.5px] font-black uppercase tracking-widest text-slate-405">En Pause</span>
            </div>
            <div className={`p-4 rounded-[28px] border ${darkMode ? 'bg-zinc-900/30 border-zinc-900' : 'bg-white border-slate-100'} flex flex-col items-center justify-center text-center space-y-1`}>
              <div className="flex items-center space-x-1.5">
                <div className="w-2 h-2 rounded-full bg-slate-400" />
                <span className="text-sm font-black italic text-slate-400 leading-none">{numOff}</span>
              </div>
              <span className="text-[7.5px] font-black uppercase tracking-widest text-slate-405">Inactifs</span>
            </div>
          </div>

          {/* TEAM MEMBERS LIST CARD */}
          <div className={`${darkMode ? 'bg-zinc-900/30' : 'bg-white'} p-6 rounded-[32px] shadow-sm border ${darkMode ? 'border-zinc-900' : 'border-slate-100'} space-y-4`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                <div className="p-2 bg-emerald-500/10 text-[#059669] rounded-xl">
                  <Users size={18} />
                ...
                <div className="text-left">
                  <h3 className="text-[11px] font-black uppercase italic tracking-wider leading-none">Supervision en direct</h3>
                  <p className="text-[7px] text-slate-400 uppercase font-bold mt-1">Feuilles de temps par lien magique</p>
                </div>
              </div>
              <button 
                onClick={() => { setVista('heures-paie'); playNotificationSound(); }}
                className={`px-3 py-1.5 rounded-xl border text-[7.5px] font-black uppercase italic ${darkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-300 hover:text-white hover:bg-zinc-900' : 'bg-slate-50 border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
              >
                + Ajouter S-T
              </button>
            </div>
            ...
            <div className="space-y-3">
              {collaborateurs.map((emp) => {
                const isWorking = emp.status === 'working';
                const isPaused = emp.status === 'paused';
                const avatarBg = isWorking ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30' : isPaused ? 'bg-amber-500/10 text-amber-500 border-amber-500/30' : 'bg-slate-200 text-slate-500 border-slate-300 dark:bg-zinc-900';
                
                return (
                  <div 
                    key={emp.id}
                    className={`p-4 rounded-3xl border transition-all ${darkMode ? 'bg-zinc-950/40 border-zinc-900/60' : 'bg-slate-50/50 border-slate-150/60'} flex flex-col space-y-3 text-left`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        <div className={`w-10 h-10 rounded-full border flex items-center justify-center font-black italic text-xs ${avatarBg}`}>
                          {emp.name.split(' ').map((n: string) => n[0]).join('')}
                        </div>
                        <div>
                          <p className="text-[11px] font-black uppercase italic tracking-tight text-slate-800 dark:text-zinc-100">{emp.name}</p>
                          <p className="text-[7.5px] font-bold text-slate-400 uppercase tracking-tight">{emp.job} • {emp.rate}$/h</p>
                        </div>
                      </div>

                      <div className="flex flex-col items-end">
                        {isWorking && (
                          <span className="bg-emerald-500/10 text-[#059669] text-[7px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full flex items-center space-x-1 border border-emerald-500/20">
                            <span className="w-1.5 h-1.5 bg-[#059669] rounded-full animate-pulse" />
                            <span>En Service ({formatTime(emp.timerElapsedSeconds || 0)})</span>
                          </span>
                        )}
                        {isPaused && (
                          <span className="bg-amber-500/10 text-amber-500 text-[7px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full flex items-center space-x-1 border border-amber-500/20">
                            <span className="w-1.5 h-1.5 bg-amber-500 rounded-full" />
                            <span>En Pause ({formatTime(emp.timerElapsedSeconds || 0)})</span>
                          </span>
                        )}
                        {emp.status === 'off' && (
                          <span className="bg-slate-100 dark:bg-zinc-900 text-slate-400 text-[6.5px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">
                            Hors service
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-zinc-900/50">
                      <span className="text-[7px] font-black text-slate-400 dark:text-zinc-500 uppercase">Securité BYOS</span>
                      
                      <button
                        onClick={() => handleGenerateMagicLink(emp)}
                        className="py-1.5 px-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[8px] uppercase tracking-wider italic rounded-xl border-none transition-all cursor-pointer flex items-center space-x-1"
                      >
                        <Zap size={10} />
          ...
          <div className={`${darkMode ? 'bg-zinc-900/30' : 'bg-white'} p-6 rounded-[32px] shadow-sm border ${darkMode ? 'border-zinc-900' : 'border-slate-100'} space-y-4`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <div className="text-left">
                  <h3 className="text-[11px] font-black uppercase italic tracking-wider leading-none">Rapports GPS & Traçabilité Fiscale</h3>
                  <p className="text-[7px] text-slate-400 uppercase font-bold mt-1">Conformité de présence légale de l'équipe</p>
                </div>
              </div>
              <span className="text-[6.5px] font-black uppercase bg-indigo-500/10 text-indigo-500 px-2.5 py-1 rounded">AUDIT ACTIF</span>
            </div>

            <div className="space-y-2.5 max-h-[220px] overflow-y-auto">
              {gpsLogs.map((log) => (
                <div key={log.id} className="p-3 rounded-2xl border border-slate-105 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/40 text-left flex justify-between items-start text-[8px] transition-all">
                  <div>
                    <div className="flex items-center space-x-1.5">
                      <span className="font-black text-slate-800 dark:text-zinc-100">{log.employeeName}</span>
                      <span className="text-slate-400">•</span>
                      <span className="font-bold text-slate-500 uppercase tracking-tight">{log.action}</span>
                    </div>
                    <p className="text-slate-400 dark:text-zinc-500 mt-1">{log.time} • <span className="font-mono text-[7.5px]">{log.coords}</span></p>
                  </div>
                  <span className="bg-emerald-500/10 text-[#059669] px-2.5 py-1 rounded-md font-black uppercase">{log.city}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <AnimatePresence>
          {magicLinkModal && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              <div onClick={() => setMagicLinkModal(null)} className="absolute inset-0 bg-black/65 backdrop-blur-sm" />
              
              <motion.div 
                initial={{ opacity: 0, scale: 0.95, y: 15 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 15 }}
                className={`relative z-10 w-full max-w-sm p-6 rounded-[32px] shadow-2xl text-left border ${darkMode ? 'bg-zinc-950 border-zinc-900 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="text-left">
                    <span className="text-[6.5px] font-black uppercase bg-emerald-500/10 text-[#059669] tracking-widest px-2.5 py-1 rounded-md italic">GENERATOR BYOS TOKEN</span>
                    <h3 className="text-base font-black uppercase italic tracking-tighter mt-1.5 leading-none">Lien Magique Actif</h3>
                  </div>
                  <button onClick={() => setMagicLinkModal(null)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-900 text-slate-400">
                    <X size={18} />
                  </button>
                </div>

                <div className={`p-4 rounded-2xl mb-4 text-xs font-bold leading-normal text-left ${darkMode ? 'bg-zinc-900 text-zinc-300' : 'bg-slate-50 text-slate-700'}`}>
                  <p className="text-[8px] text-slate-400 uppercase tracking-wider block mb-1 pl-1">Employé désigné</p>
                  <p className="text-sm font-black text-[#1F2937] dark:text-white uppercase italic tracking-tight mb-2 pl-1">{magicLinkModal.name}</p>
                  <p className="text-[7.5px] text-zinc-400 leading-relaxed font-normal mb-3 pl-1">
                    L'employé recevra ce lien sécurisé par SMS ou courriel. En cliquant dessus, il se connectera sans mot de passe à un portail de présence restreint et géolocalisé.
                  </p>
                  
                  <div className={`p-3 rounded-xl border flex items-center justify-between space-x-2 ${darkMode ? 'bg-black border-zinc-800' : 'bg-white border-slate-200'}`}>
                    <span className="font-mono text-[8.5px] text-emerald-500 font-bold truncate max-w-[200px]">{magicLinkModal.url}</span>
                    <button 
                      onClick={() => {
                        navigator.clipboard.writeText(magicLinkModal.url);
                        alert("📋 Lien magique sécurisé copié dans le presse-papiers !");
                      }}
                      className="p-1.5 text-slate-400 hover:text-[#059669] transition-colors border-none bg-transparent cursor-pointer"
                ...
                <button
                  onClick={() => {
                    setEmployeeMagicSession(magicLinkModal.id);
                    setMagicLinkModal(null);
                    playNotificationSound();
                  }}
                  className="w-full py-4 bg-[#059669] hover:bg-emerald-600 text-white rounded-2xl text-[9.5px] font-black uppercase italic tracking-widest shadow-lg flex items-center justify-center space-x-1.5 border-none cursor-pointer"
                >
                  <Zap size={14} />
                  <span>Simuler Connexion Salarié</span>
                </button>
              </motion.div>
            </div>
          )}
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-black text-zinc-100' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans text-left animate-in zoom-in-95 max-w-full overflow-x-hidden md:pl-72 relative transition-all duration-300`}>
                ...
                <tbody className={`divide-y ${darkMode ? 'divide-zinc-900' : 'divide-slate-100'}`}>
                   {(tabReporte === 'ventes' ? processedHistorique : processedDepenses)
                     ...
                     <tr key={item.id} className={`text-[10px] font-bold transition-colors group ${darkMode ? 'bg-black hover:bg-zinc-950' : 'bg-white hover:bg-slate-50'}`}>
                      ...
                      <td className={`py-5 px-6 font-black italic tracking-tighter ${darkMode ? 'text-zinc-100' : 'text-slate-900'}`}>{(item as any).cliente || (item as any).fournisseur}</td>
      ...
      <div className={`p-4 border-t space-y-3 ${darkMode ? 'bg-black border-zinc-900' : 'bg-white border-slate-200'}`}>
        ...
        <div className="flex gap-2">
          <button 
            onClick={() => {
              setNewTxData({
                fecha: new Date().toISOString().split('T')[0],
                tiers: '',
                cat: tabReporte === 'ventes' ? (activeCompanyId === '1' ? 'Honoraires de gestion immobilière' : 'Ventes') : 'À classer',
                total: '',
          ...
          >
            <Plus size={16} />
            <span>Nouveau {tabReporte === 'ventes' ? 'Revenu' : 'Frais'}</span>
          </button>

          <button 
            onClick={exportarAContador} 
            disabled={exportStatus === 'loading'}
            className={`flex-1 py-4 text-white rounded-[28px] text-[10px] font-black uppercase flex items-center justify-center space-x-2 shadow-lg transition-all tracking-widest italic ${exportStatus === 'loading' ? 'bg-slate-400 cursor-not-allowed' : (darkMode ? 'bg-zinc-900 border border-zinc-800 hover:bg-zinc-850 text-zinc-300' : 'bg-slate-900 text-white hover:bg-slate-800')}`}
          >
        ...
        <main className="p-6 space-y-6 max-w-5xl mx-auto w-full flex-1 relative z-10 text-left">
          ...
          <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
             ...
             <button 
               onClick={() => {
                 playNotificationSound();
                 setRapportSentSuccess(false);
                 setShowComptableModal(true);
               }}
               className="flex-1 py-4.5 rounded-2xl text-[9px] font-black uppercase italic tracking-widest bg-gradient-to-r from-teal-600 via-emerald-600 to-teal-505 text-white shadow-lg shadow-emerald-900/20 flex items-center justify-center space-x-2 transition-all hover:scale-[1.01] active:scale-98"
             >
                <Mail size={16} />
               ...
               >
                  ...
                  {rapportSentSuccess ? (
                    ...
                    <div className="space-y-4 text-left">
                       ...
                       <div className="text-left">
                          <label className="block text-[8px] font-black uppercase italic text-slate-400 tracking-wider mb-1.5">Courriel de la comptable (Natalia C.)</label>
                          <input 
                            type="email"
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            ...
            <div className="flex items-center space-x-2">
              <button 
                onClick={() => {
                  alert("🔒 Fonctionnalité Premium: Passez au Forfait Premium à 18.99$/mois pour déverrouiller la synchronisation bancaire en direct en tiempo real (Inclus: Connexion Flinks/Plaid, multi-socios y automatización total). Votre forfait de Base à 4.99$/mois (5.74$ avec taxes) inclut uniquement l'importation de relevés PDF.");
                }}
