...
import {
  ...
  Folder, FolderOpen, ChevronRight, Zap, ShieldCheck, Eye, Upload, Plus, FileSearch, FileQuestion, Home
} from 'lucide-react';
...
export default function DossierFiscauxView({ darkMode, setVista, playNotificationSound, sidebarToggle }: DossierFiscauxViewProps) {
  ...
  // Initial Mock File Database inside the secure AutoCompt Vault (Dossiers)
  ...
  const [newDocProfile, setNewDocProfile] = useState<'Solutions GPA' | 'Triplex'>('Solutions GPA');
  ...
  const getFilesCountForProfile = (year: number, profile: 'Solutions GPA' | 'Triplex') => {
    return dossierFiles.filter(f => f.year === year && f.profile === profile).length;
  };

  const getFilesCountForCategory = (year: number, profile: 'Solutions GPA' | 'Triplex', cat: string) => {
            ...
            <div className="space-y-4 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {categoriesGroup.map(cat => {
                  const itemsCount = getFilesCountForCategory(currentYearFolder, currentProfileFolder as 'Solutions GPA' | 'Triplex', cat);
              ...
              >
                ...
                <div className={`p-8 md:w-1/2 flex flex-col justify-between border-b md:border-b-0 md:border-r ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-slate-100 border-slate-200'}`}>
                  <div>
                    ...
                    {/* Simulated paper voucher layout */}
                    <div className="bg-white text-slate-900 p-6 rounded-[28px] shadow-inner space-y-4 font-sans min-h-[320px] border border-slate-200">
