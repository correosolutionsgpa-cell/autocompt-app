 ...
 * ContratsDLShell.tsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Rama: Rama_Gestionnaires (Contrats & Résolutions DocuLegal)
 * Extraído de: src/App.tsx (L18218–L18279) — Fase 13 del desmantelamiento modular
 *
 * Shell de navigation de 63 líneas que envuelve a <SyndicatDocuLegal>.
 * SyndicatDocuLegal es un componente externo en src/components/SyndicatDocuLegal.
...
export interface ContratsDLShellProps {
  // Mode
  darkMode: boolean;

  // Profil admin (header display)
  adminName: string;
  adminRole: string;
  adminPhoto: string;
...
const ContratsDLShell: React.FC<ContratsDLShellProps> = ({
  darkMode,
  adminName,
  adminRole,
  adminPhoto,
      ...
      <header className={`${darkMode ? "bg-slate-900/40 border-white/[0.08] shadow-[inset_0_1px_1px_rgba(255,255,255,0.06),0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-md" : "bg-white border-slate-200"} px-6 py-4 border-b shadow-sm sticky top-0 z-50 flex items-center justify-between`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center gap-2.5 bg-slate-50/50 dark:bg-zinc-900/40 p-1.5 pr-3 rounded-full border border-slate-150 dark:border-zinc-800 shadow-sm hover:border-teal-500/30 transition-all cursor-pointer">
            <img src={adminPhoto} alt={adminName} className="w-7 h-7 rounded-full border border-violet-500/20 object-cover shadow-sm" />
...
export default ContratsDLShell;
